...
contract StakingRewards {
    ...
    function lastTimeRewardApplicable() public view returns (uint256) {
        return _min(finishAt, block.timestamp); // x<=y?x:y
    }

    function rewardPerToken() public view returns (uint256) {
        //Checks if there are any staked token in the contract
        if (totalSupply == 0) {
            return rewardPerTokenStored; //last calculated reward per staked token
        }
    ...
    function stake(uint256 _amount) external updateReward(msg.sender) {
        ...
        balanceOf[msg.sender] += _amount; // User address => staked amount
        totalSupply += _amount;
    }
